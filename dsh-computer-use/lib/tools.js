...
import { ComputerUseSidecar } from "./sidecar.js";
...
async function requirePermission(config) {
...
export async function startPermissionWidget(getConfig) {
    const config = getConfig();
    if (!config.permissionWidgetEnabled)
        return;
    const sidecarInstance = getSidecar(getConfig);
    try {
        await sidecarInstance.request('permission_widget_show', { permissionFile: permissionFilePath(config) }, config, undefined);
    }
    catch {
...
async function attachScreenshot(ctx, path) {
...
async function maybeApprove(ctx, exec, config, reason) {
    if (!config.requireApproval)
        return;
    if (exec.agent === undefined)
        return; // programmatic calls skip interactive approval
    ...
    if (policy === 'never') {
        if (config.skipApprovalWhenPolicyNever)
            return;
        throw new Error('当前会话审批策略为“不再询问”（never），且 skipApprovalWhenPolicyNever=false，此操作被拒绝。');
    }
...
let sidecar = null;
function getSidecar(getConfig) {
    if (sidecar === null)
        sidecar = new ComputerUseSidecar(getConfig);
    return sidecar;
}
/** Public cleanup called from the plugin disposer. */
export function disposeSidecar() {
    sidecar?.close();
    sidecar = null;
}
...
async function ensureOverlay(deps, exec) {
    const config = deps.getConfig();
    if (!config.overlayEnabled)
        return;
    try {
        await getSidecar(deps.getConfig).request('overlay_begin', {
...
// ------------------------------------------------------------------ tools
export function defineListWindowsTool({ ctx, getConfig }) {
    return defineTool({
        name: 'computer_list_apps',
        description: 'computer control / desktop automation: 列出当前 Windows 桌面上可操作的应用窗口（窗口ID、标题、类名、位置）。返回的 windowId 用于 computer_get_window_state 等后续操作。',
        parameters: {},
        output: {
            schema: { type: 'json' },
            render(_args, value) {
        ...
        async execute(_args, exec) {
            ...
            const result = await getSidecar(getConfig).request('list_windows', {}, config, exec.signal);
            return asJson(result);
        },
    });
}
export function defineGetWindowStateTool({ ctx, getConfig }) {
    return defineTool({
        name: 'computer_get_window_state',
        description: 'computer control / desktop automation: 获取某个窗口的最新状态：截图（保存为附件，vision 模型可用）和/或可访问性 UI 树（带元素索引）。动作后必须重新调用本工具刷新，因为元素索引和坐标只对当次观察有效。',
        parameters: {
        ...
        output: {
            ...
            render(_args, value) {
                ...
                if (v.screenshotAttachment !== undefined) {
                    blocks.push({ type: 'image', attachment: v.screenshotAttachment });
                }
                const parts = [`窗口：${v.title ?? ''}`];
                if (v.accessibilityTree !== undefined && v.accessibilityTree !== '') {
                    parts.push(`\n可访问性树：\n${v.accessibilityTree}`);
                }
                if (v.screenshotPath !== undefined)
        ...
        async execute(args, exec) {
            ...
            await ensureOverlay({ ctx, getConfig }, exec);
            const call = args;
            if (typeof call.windowId !== 'number')
                throw new Error('windowId 必须是数字');
            ...
            const raw = await getSidecar(getConfig).request('get_window_state', {
            ...
            if (raw.accessibilityTree !== undefined)
                value.accessibilityTree = raw.accessibilityTree;
            return asJson(value);
        },
    });
}
export function defineActivateWindowTool({ ctx, getConfig }) {
    return defineTool({
        ...
        output: {
            ...
            render(_args, value) {
                const v = value;
                return [{ type: 'text', text: `已激活窗口 ${v.windowId ?? ''}（${v.activated ? '成功' : '失败'}）` }];
            },
        },
        timeoutMs: 60_000,
        isConcurrencySafe: () => false,
...
export function defineClickTool({ ctx, getConfig }) {
    return defineTool({
        name: 'computer_click',
        description: 'computer control / desktop automation / mouse: 在窗口内点击：传 elementIndex 点击 UI 树中的元素，或传 x/y 窗口相对坐标点击。button: left/right/middle，clickCount 默认为 1。硬性安全限制：禁止对 DSH 聊天窗口（DeepSeek Harness）点击，否则会覆盖聊天框。',
        parameters: {
            windowId: { type: 'number', required: true, description: 'computer_list_apps 返回的窗口 ID' },
        ...
        output: {
            schema: { type: 'json' },
            render(_args, value) {
                return [{ type: 'text', text: `点击完成：${JSON.stringify(value)}` }];
            },
        },
        timeoutMs: 120_000,
        isConcurrencySafe: () => false,
        async execute(args, exec) {
            ...
            const params = { windowId: call.windowId };
            if (call.elementIndex !== undefined)
                params.elementIndex = call.elementIndex;
            if (call.x !== undefined)
                params.x = call.x;
            if (call.y !== undefined)
                params.y = call.y;
            if (call.mouseButton !== undefined)
                params.mouseButton = call.mouseButton;
            if (call.clickCount !== undefined)
                params.clickCount = call.clickCount;
            const result = await getSidecar(getConfig).request('click', params, config, exec.signal);
            return asJson(result);
        },
    });
}
export function defineTypeTextTool({ ctx, getConfig }) {
    return defineTool({
        ...
        output: {
            ...
            render(_args, value) {
                const v = value;
                return [{ type: 'text', text: `已输入 ${v.chars ?? 0} 个字符（${v.typed ? '成功' : '失败'}）` }];
            },
        },
        timeoutMs: 120_000,
        isConcurrencySafe: () => false,
...
export function definePressKeyTool({ ctx, getConfig }) {
    return defineTool({
        name: 'computer_press_key',
        description: 'computer control / desktop automation / keyboard: 发送键盘组合键（press hotkey / shortcut），例如 "Control_L+a"、"Control_L+Shift_L+Tab"、"Return"、"F5"。禁止使用 Windows/Meta 键。硬性安全限制：禁止对 DSH 聊天窗口按键，以免覆盖聊天框。',
        parameters: {
            windowId: { type: 'number', required: true, description: 'computer_list_apps 返回的窗口 ID' },
        ...
        output: {
            schema: { type: 'json' },
            render(_args, value) {
                return [{ type: 'text', text: `按键完成：${JSON.stringify(value)}` }];
            },
        },
        timeoutMs: 120_000,
        isConcurrencySafe: () => false,
...
export function defineScrollTool({ ctx, getConfig }) {
    return defineTool({
        ...
        output: {
            schema: { type: 'json' },
            render(_args, value) {
                return [{ type: 'text', text: `滚动完成：${JSON.stringify(value)}` }];
            },
        },
        timeoutMs: 120_000,
        isConcurrencySafe: () => false,
        async execute(args, exec) {
            ...
            const result = await getSidecar(getConfig).request('scroll', { windowId: call.windowId, x: call.x, y: call.y, scrollX: call.scrollX ?? 0, scrollY: call.scrollY ?? 0 }, config, exec.signal);
            return asJson(result);
        },
    });
}
export function defineDragTool({ ctx, getConfig }) {
    return defineTool({
        name: 'computer_drag',
        ...
        output: {
            schema: { type: 'json' },
            render(_args, value) {
                return [{ type: 'text', text: `拖动完成：${JSON.stringify(value)}` }];
            },
        },
        timeoutMs: 120_000,
        isConcurrencySafe: () => false,
        async execute(args, exec) {
            ...
            const result = await getSidecar(getConfig).request('drag', { windowId: call.windowId, fromX: call.fromX, fromY: call.fromY, toX: call.toX, toY: call.toY }, config, exec.signal);
            return asJson(result);
        },
    });
}
export function defineLaunchAppTool({ ctx, getConfig }) {
    return defineTool({
        ...
        output: {
            schema: { type: 'json' },
            render(_args, value) {
                return [{ type: 'text', text: `启动应用：${JSON.stringify(value)}` }];
            },
        },
        timeoutMs: 60_000,
        isConcurrencySafe: () => false,
        async execute(args, exec) {
            ...
            await ensureOverlay({ ctx, getConfig }, exec);
            const call = args;
            if (typeof call.app !== 'string' || call.app.trim() === '')
                throw new Error('app 不能为空');
            await maybeApprove(ctx, exec, config, `启动应用 ${call.app}`);
            const result = await getSidecar(getConfig).request('launch_app', { app: call.app.trim(), args: Array.isArray(call.args) ? call.args.map(String) : [] }, config, exec.signal);
            return asJson(result);
        },
    });
}
