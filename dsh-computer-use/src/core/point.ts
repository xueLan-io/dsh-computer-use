...
export function point(
  ...
  if (coordinateSpace === 'screen') {
    if (
      ...
      throw new ComputerUseError(
        ...
        `Screen coordinates (${x}, ${y}) are outside the target window`,
        'REQUIRES_REFRESH',
      )
    }
    return { x, y }
  ...
  if (
    ...
    throw new ComputerUseError(
      'COORDINATE_OUT_OF_BOUNDS',
      `Coordinates (${x}, ${y}) are outside the window bounds ${window.width}x${window.height}`,
      'REQUIRES_REFRESH',
    )
  }
...
export function validateElementIndex(index: number): void {
