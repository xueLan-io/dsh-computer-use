...
export function capabilityUnavailable(capability: keyof Capabilities): Error {
 ...
 */
export const META_KEY_TOKENS: ReadonlySet<string> = new Set([
...
export function assertCapability(state: CapabilityState, capability: keyof Capabilities): void {
  if (state === false) throw capabilityUnavailable(capability)
}
