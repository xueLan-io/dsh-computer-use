...
import { brandProtected } from './protection.ts'
...
export function sameIdentity(a: DesktopWindow, b: DesktopWindow): boolean {
  return (
...
export function createObservation(
  ...
  value: Partial<Omit<Observation, keyof DesktopWindow | 'observationId' | 'createdAt' | 'expiresAt' | 'sessionId' | 'agentId'>> = {},
  metadata: ObservationOwner,
): Observation {
...
async function validateAccessibilityTree(
  old: Observation,
  windowId: WindowId,
  ...
  if (!old.uiaChecksum) {
    throw new ComputerUseError(
      ...
      `Action ${action} requires a UI Automation observation; please re-observe the window`,
      'REQUIRES_REFRESH',
    )
  }
  let current: Awaited<ReturnType<DesktopProvider['accessibilityTree']>>
  try {
  ...
  if (
    current.checksum !== old.uiaChecksum ||
    ...
    throw new ComputerUseError(
      'UIA_TREE_CHANGED',
      `Window UI Automation tree changed; cannot perform ${action}`,
      'REQUIRES_REFRESH',
    )
  }
}
...
export async function validateObservation(
  ...
  if (!old) throw new ComputerUseError('INVALID_OBSERVATION', 'Invalid observationId; please re-observe', 'REQUIRES_REFRESH')
...
export function __clearObservationsForTest(): void {
  observations.clear()
}

/** Current observation count (diagnostics/tests only). */
export function __observationCountForTest(): number {
